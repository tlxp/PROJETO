...
// Testes unitários da lógica de análise (VM, API, pseudo-C e relatório estático).
...
describe("Parsing do relatório estático", () => {
...
describe("API e normalização", () => {
  describe("buildAnalysisResultFromJob", () => {
    ...
    it("mantém dynamicPending quando both tem dynamicResult sem relatório e job completed", () => {
      const job = {
        status: "completed",
        analysisType: "both",
        staticResult: {
          report: "Relatório estático",
          cCode: "void main(){}",
          ilCode: "",
          fileName: "sample.exe",
          riskScore: 10,
      ...
      const res = buildAnalysisResultFromJob(job);
      expect(res?.dynamicPending).toBe(true);
      expect(res?.report).toBe("Relatório estático");
    });

    it("liberta dynamicPending quando relatório VM chega em job both", () => {
      const job = {
        status: "completed",
        analysisType: "both",
        staticResult: { report: "R", cCode: "c", ilCode: "", fileName: "x", riskScore: 0, riskLevel: "" },
        dynamicResult: { dynamicReportText: "Relatório VM completo" },
      };
      const res = buildAnalysisResultFromJob(job);
      expect(res?.dynamicPending).toBe(false);
      expect(res?.vmReport).toBe("Relatório VM completo");
    });
  });
