...
if ([string]::IsNullOrWhiteSpace($WindowsIsoPath) -or -not (Test-Path -LiteralPath $WindowsIsoPath)) {
    throw "ISO nao encontrado em '$WindowsIsoPath'. Edite WindowsIsoPath em _Config.ps1."
}
...
try {
    ...
    if ($isoLang) {
        ...
        Write-Host '      Idioma default do ISO: (nao determinado - nao foi possivel ler Default= em lang.ini)'
...
try {
    ...
    if ($uiLangs -and ($uiLangs -contains 'en-US')) {
        ...
        $listed = if ($uiLangs) { ($uiLangs -join ', ') } else { '(nao foi possivel ler sources\lang.ini)' }
        throw ('ISO sem en-US. Idiomas detectados: ' + $listed)
    }
} catch {
    throw (
        'ISO invalida para este projeto: e necessaria uma ISO do Windows em en-US. ' +
