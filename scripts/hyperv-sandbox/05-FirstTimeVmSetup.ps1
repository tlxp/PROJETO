...
Write-Host "=== Primeira entrada na VM: instalar software comum e criar snapshot ==="
Write-Host ""

$vm = Get-VM -Name $VMName -ErrorAction SilentlyContinue
if (-not $vm) {
    Write-Host "[ERRO] VM '$VMName' não encontrada. Execute primeiro 01-Setup-MalwareSandbox.ps1 e instale o Windows na VM."
    exit 1
}

# 1) Se a VM estiver ligada, parar para começar do estado limpo
if ($vm.State -eq "Running") {
    Write-Host "[1/6] A parar a VM..."
    Stop-VM -Name $VMName -Force -ErrorAction SilentlyContinue
    Start-Sleep -Seconds 5
} else {
    Write-Host "[1/6] VM já está desligada."
}

# 2) Arrancar VM
Write-Host "[2/6] A arrancar a VM..."
Start-VM -Name $VMName | Out-Null
$bootWait = 90
Write-Host "      A aguardar $bootWait s pelo arranque do Windows..."
Start-Sleep -Seconds $bootWait

# 3) Ativar Guest Service
Write-Host "[3/6] A ativar Guest Service Interface..."
...
if (-not (Test-Path $prepareScript)) {
    Write-Host "[ERRO] Script não encontrado: $prepareScript"
    exit 1
}

$guestPath = "$VMScriptsPath\Prepare-RealisticEnvironment.ps1"
Write-Host "[4/6] A copiar Prepare-RealisticEnvironment.ps1 para a VM..."
Copy-VMFile -VMName $VMName -SourcePath $prepareScript -DestinationPath $guestPath -CreateFullPath -FileSource Host -ErrorAction Stop
Write-Host "      A executar na VM: Prepare-RealisticEnvironment.ps1 -InstallCommonSoftware (winget)..."
try {
    ...
    Write-Host "      [AVISO] Invoke-Command falhou: $_"
    Write-Host "      Se a VM não suportar PowerShell Direct, execute manualmente na VM:"
    Write-Host "        cd C:\analysis_work; .\Prepare-RealisticEnvironment.ps1 -InstallCommonSoftware"
    Write-Host "      Depois pare a VM e execute este script novamente para criar o snapshot."
}

# 5) Parar a VM
Write-Host "[5/6] A parar a VM..."
Stop-VM -Name $VMName -Force -ErrorAction SilentlyContinue
Start-Sleep -Seconds 8

# 6) Criar ou atualizar snapshot CleanState
Write-Host "[6/6] A criar/atualizar snapshot '$SnapshotName'..."
...
Write-Host ""
Write-Host "=== Primeira entrada concluída. Pode agora usar 04-Run-Sample.ps1 para analisar amostras. ==="
